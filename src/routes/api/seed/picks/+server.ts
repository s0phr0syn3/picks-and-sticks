...
export const GET: RequestHandler = async () => {
  try {
    ...
    const weeks = db.select({week: schedules.week}).from(schedules).groupBy(schedules.week).orderBy(schedules.week).all()

    const pickRecords: { week: number; round: number; userId: number; teamId: number; orderInRound: number; assignedById?: number; }[] = []
    const rounds = 4
    ...
    for (const week of weeks.map(x => x.week)) {
      ...
      for (let round = 1; round <= rounds; round++) {
        const roundUserIds = round % 2 === 1 ? shuffledUserIds.map(user => user.id) : [...shuffledUserIds.map(user => user.id).reverse()]
